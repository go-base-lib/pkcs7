...
var x509SignatureAlgorithmDetails = []struct {
...
type Operation interface {
	// PKCS7Info 获取原始的pkcs7对象
	PKCS7Info() interface{}
	// Verify 验证签名
	Verify() error
	// VerifyWithOptions 验证签名伴随参数
	VerifyWithOptions(options *VerifyOptions) error
	// Content 内容
	Content() []byte
	// SetContent 设置内容
	SetContent(c []byte)

	UnmarshalSignedAttribute(attributeType asn1.ObjectIdentifier, out interface{}) error
}

type sm2PKCS7Operation struct {
	*sm2X509.PKCS7
}

func (s *sm2PKCS7Operation) Content() []byte {
	return s.PKCS7.Content
}

func (s *sm2PKCS7Operation) SetContent(c []byte) {
	s.PKCS7.Content = c
}

func (s *sm2PKCS7Operation) VerifyWithOptions(options *VerifyOptions) error {

	if len(options.CertData) > 0 {
		certificates, err := sm2X509.ParseCertificates(options.CertData)
		if err != nil {
	...
	if len(options.SrcContentHash) == 0 {
		if len(options.SrcContent) > 0 {
			s.PKCS7.Content = options.SrcContent
		}
	...
	if len(s.Certificates) == 0 {
		return errors.New("证书看列表不存在")
	}

	for i := range s.Signers {
		if err := s.verifySignatureWithContentHash(s.PKCS7, i, options.SrcContentHash); err != nil {
			return err
		}
	}
...
func (s *sm2PKCS7Operation) getHashForOID(oid asn1.ObjectIdentifier) (sm2X509.Hash, error) {
	switch {
	case oid.Equal(oidDigestAlgorithmSHA1):
		return sm2X509.SHA1, nil
	case oid.Equal(oidSHA256):
		return sm2X509.SHA256, nil
	case oid.Equal(oidSM3):
	case oid.Equal(oidHashSM3):
		return sm2X509.SM3, nil
	}
	return sm2X509.Hash(0), sm2X509.ErrPKCS7UnsupportedAlgorithm
}

func (s *sm2PKCS7Operation) verifySignatureWithContentHash(p7 *sm2X509.PKCS7, signerIndex int, contentHash []byte) (err error) {
	signer := p7.Signers[signerIndex]
	signedData := p7.Content
	hash, err := s.getHashForOID(signer.DigestAlgorithm.Algorithm)
	if err != nil {
		return err
	}
	...
	var cert *sm2X509.Certificate
	for _, c := range s.Certificates {
		ias := signer.IssuerAndSerialNumber
		if c.SerialNumber.Cmp(ias.SerialNumber) == 0 && bytes.Compare(cert.RawIssuer, ias.IssuerName.FullBytes) == 0 {
			cert = c
		}
	}
	if cert == nil {
	...
	algo := s.getSignatureAlgorithmByHash(hash, signer.DigestEncryptionAlgorithm.Algorithm)
	if algo == sm2X509.UnknownSignatureAlgorithm {
		return sm2X509.ErrPKCS7UnsupportedAlgorithm
	}
	return cert.CheckSignature(algo, signedData, signer.EncryptedDigest)
}

func (s *sm2PKCS7Operation) getSignatureAlgorithmByHash(hash sm2X509.Hash, oid asn1.ObjectIdentifier) sm2X509.SignatureAlgorithm {
	switch hash {
	case sm2X509.SM3:
		switch {
		case oid.Equal(oidSM3withSM2):
			return sm2X509.SM2WithSM3
		}
	case sm2X509.SHA256:
		switch {
		case oid.Equal(oidDSASM2):
			return sm2X509.SM2WithSHA256
		}
	}
	return sm2X509.UnknownSignatureAlgorithm
}

func (s *sm2PKCS7Operation) PKCS7Info() interface{} {
	return s.PKCS7
}
...
type OtherPKCS7Operation struct {
	*PKCS7
}

func (o *OtherPKCS7Operation) Content() []byte {
	return o.PKCS7.Content
}

func (o *OtherPKCS7Operation) SetContent(c []byte) {
	o.PKCS7.Content = c
}

func (o *OtherPKCS7Operation) VerifyWithOptions(options *VerifyOptions) error {
	if len(options.CertData) > 0 {
		certificates, err := x509.ParseCertificates(options.CertData)
		if err != nil {
			return errors.New("证书解析失败")
		}
	...
	if len(options.SrcContentHash) > 0 {
		return o.VerifyWithSrcHash(options.SrcContentHash)
	}

	if len(options.SrcContent) > 0 {
		o.PKCS7.Content = options.SrcContent
	}

	return o.Verify()
}

func (o *OtherPKCS7Operation) PKCS7Info() interface{} {
	return o.PKCS7
}
...
func Parse(data []byte) (p7 Operation, err error) {
	if len(data) == 0 {
	...
	default:
		r, e := sm2X509.ParsePKCS7(data)
		if e == nil {
			return &sm2PKCS7Operation{
				PKCS7: r,
			}, nil
		}
...
func parseSignedData(data []byte) (*OtherPKCS7Operation, error) {
	var sd signedData
...
func parseEnvelopedData(data []byte) (*OtherPKCS7Operation, error) {
...
// this time.
func (p *PKCS7) Verify() (err error) {
	if len(p.Signers) == 0 {
		return errors.New("pkcs7: Message has no signers")
	}
	for _, signer := range p.Signers {
		if err := verifySignature(p, signer); err != nil {
			return err
		}
	}
	return nil
...
func (p *PKCS7) VerifyWithSrcHash(contentHash []byte) (err error) {
	if len(p.Signers) == 0 {
		return errors.New("pkcs7: Message has no signers")
	}
	for _, signer := range p.Signers {
		if err := verifySignatureWithHash(p, signer, contentHash); err != nil {
			return err
		}
	}
	return nil
...
func verifySignatureWithHash(p7 *PKCS7, signer signerInfo, contentHash []byte) error {
	...
	return checkSignature(algo, signedData, signer.EncryptedDigest, cert.PublicKey)
}
...
func (p *PKCS7) GetOnlySigner() *x509.Certificate {
	if len(p.Signers) != 1 {
		return nil
	}
	signer := p.Signers[0]
	return getCertFromCertsByIssuerAndSerial(p.Certificates, signer.IssuerAndSerialNumber)
}
...
func (p *PKCS7) Decrypt(cert *x509.Certificate, pk crypto.PrivateKey) ([]byte, error) {
	data, ok := p.raw.(envelopedData)
	if !ok {
...
func (p *PKCS7) UnmarshalSignedAttribute(attributeType asn1.ObjectIdentifier, out interface{}) error {
	sd, ok := p.raw.(signedData)
	if !ok {
